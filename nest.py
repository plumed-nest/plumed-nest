...
def add_readme(file, tested, success, exe, has_load, has_custom):
    with open("README.md","a") as o:
        ...
        if has_load:
            badge += ' [![with LOAD](https://img.shields.io/badge/with-LOAD-yellow.svg)]({{ "/" | absolute_url }}load)'
        if has_custom:
            badge += ' [![with custom code](https://img.shields.io/badge/with-custom_code-red.svg)]({{ "/" | absolute_url }}custom)'
